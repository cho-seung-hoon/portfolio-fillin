...
public class ScheduleService {
    ...
    private Page<ScheduleListResponse> convertToDetailResponsePage(Page<Schedule> schedules) {
        return schedules.map(s -> { // getNickname() -> N + 1 문제 발생 가능 -> db와 서버 사이이 통신 과부하 발생
            String mentorNickname = getNickname(s.getMentorId());
            String menteeNickname = getNickname(s.getMenteeId());
            Instant startTime = s.getScheduleTimeList().get(0).getStartTime();
    ...
    public Page<ScheduleListResponse> getMenteeSchedules(String memberId, Pageable pageable) {
        ...
        Page<Schedule> schedules = scheduleRepository.findByMenteeIdWithTimes(memberId, pageable);
    ...
    public Page<ScheduleListResponse> getMentorSchedules(String memberId, Pageable pageable) {
        ...
        Page<Schedule> schedules = scheduleRepository.findByMentorIdWithTimes(memberId, pageable);
        return convertToDetailResponsePage(schedules);
    }
}
